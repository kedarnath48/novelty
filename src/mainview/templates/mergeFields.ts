...
export function getInheritedNames(globalId: string | null, list: GlobalTemplate[]): Set<string> {
...
export function getSeriesInheritedNames(seriesId: string | null, list: SeriesTemplate[]): Set<string> {
...
export function mergeGlobalFields(
	...
	const savedOverrides = new Map(
		fields.filter((f) => inherited.has(f.name)).map((f) => [f.name, f] as const),
	);
