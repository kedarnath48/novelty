...
interface SettingsContextType {
	...
	updateGeneral: <K extends keyof Settings["general"]>(key: K, value: Settings["general"][K]) => Promise<void>;
	updateProjects: <K extends keyof Settings["projects"]>(key: K, value: Settings["projects"][K]) => Promise<void>;
	updateAssetLibrary: <K extends keyof Settings["assetLibrary"]>(key: K, value: Settings["assetLibrary"][K]) => Promise<void>;
	updateProviders: <K extends keyof Settings["providers"]>(key: K, value: Settings["providers"][K]) => Promise<void>;
	...
	saveFile: (title: string, defaultPath: string, filters: { name: string; extensions: string[] }[]) => Promise<string | null>;
...
export function SettingsProvider({ children }: { children: ReactNode }) {
	...
	const [encryptionMode, setEncryptionMode] = useState<"machine" | "password" | "none">("none");
	...
	const loadSettings = useCallback(async () => {
		...
		try {
			const [loadedSettings, locked, mode] = await Promise.all([
				...
				console.log("Settings loaded", rpc.request["settings:get-all"]()),
			]);
	...
	const updateGeneral = useCallback(async <K extends keyof Settings["general"]>(key: K, value: Settings["general"][K]) => {
		if (!settings || isLocked) return;
		const newGeneral = { ...settings.general, [key]: value };
		await updateSetting("general", newGeneral);
	}, [settings, isLocked, updateSetting]);

	const updateProjects = useCallback(async <K extends keyof Settings["projects"]>(key: K, value: Settings["projects"][K]) => {
		if (!settings || isLocked) return;
		const newProjects = { ...settings.projects, [key]: value };
		await updateSetting("projects", newProjects);
	}, [settings, isLocked, updateSetting]);

	const updateAssetLibrary = useCallback(async <K extends keyof Settings["assetLibrary"]>(key: K, value: Settings["assetLibrary"][K]) => {
		if (!settings || isLocked) return;
		const newAssetLibrary = { ...settings.assetLibrary, [key]: value };
		await updateSetting("assetLibrary", newAssetLibrary);
	}, [settings, isLocked, updateSetting]);

	const updateProviders = useCallback(async <K extends keyof Settings["providers"]>(key: K, value: Settings["providers"][K]) => {
		if (!settings || isLocked) return;
		const newProviders = { ...settings.providers, [key]: value };
		await updateSetting("providers", newProviders);
	}, [settings, isLocked, updateSetting]);

	const updateProviderConfig = useCallback(async (providerId: string, config: ProviderConfig) => {
		if (!settings || isLocked) return;
		const newConfigs = { ...settings.providers.configs, [providerId]: config };
	...
	const updateStorage = useCallback(async <K extends keyof Settings["storage"]>(key: K, value: Settings["storage"][K]) => {
		if (!settings || isLocked) return;
	...
	const openDirectory = useCallback(async (title: string): Promise<string | null> => {
		const rpc = getRPC();
		return await rpc.request["dialog:open-directory"]({ title });
	}, []);

	const openProjectFolder = useCallback(async (projectId: string): Promise<void> => {
		const rpc = getRPC();
		await rpc.request["open-project-folder"](projectId);
	}, []);

	const revealInExplorer = useCallback(async (path: string): Promise<void> => {
		const rpc = getRPC();
		await rpc.request["reveal-in-explorer"](path);
	}, []);

	const saveFile = useCallback(async (title: string, defaultPath: string, filters: { name: string; extensions: string[] }[]): Promise<string | null> => {
		const rpc = getRPC();
		return await rpc.request["dialog:save-file"]({ title, defaultPath, filters });
	}, []);

	const openFile = useCallback(async (title: string, filters: { name: string; extensions: string[] }[]): Promise<string | null> => {
		const rpc = getRPC();
