...
export default function ProjectsDialog({
	...
	const [assetPickerTargetId, setAssetPickerTargetId] = useState<string | null>(
		null,
	);
	...
	const [seriesCreateArch, setSeriesCreateArch] = useState<SeriesArchitecture>("ongoing");
	...
	const [seriesEditArch, setSeriesEditArch] = useState<SeriesArchitecture>("ongoing");
	...
	const [seriesProjects, setSeriesProjects] = useState<Array<{ id: string; name: string }>>([]);
		...
		>
			<div className={styles.toolbar}>
				...
				{!showSeries ? (
					...
					<button onClick={() => setShowSeriesCreate(true)}>New Series</button>
				)}
			</div>
			{!showSeries ? (
				loading ? (
					...
					<div className={styles.content}>
						{projects.map((project) => (
							<ProjectCard
								key={project.id}
								project={project}
								onSelect={onSelectProject ? handleSelectProject : undefined}
				...
				<div className={styles.content} style={{ display: "flex", flexDirection: "column", gap: "0.5rem", padding: "1rem" }}>
					{seriesLoading ? (
						<div>Loading...</div>
					) : seriesList.length === 0 ? (
						...
						seriesList.map((s) => (
							...
							>
								...
								<div style={{ display: "flex", gap: "0.5rem" }}>
									<button onClick={() => handleSeriesViewProjects(s)}>Projects</button>
									<button onClick={() => handleSeriesEditOpen(s)}>Edit</button>
									<button
										onClick={() => { setSeriesDeleteId(s.id); setShowSeriesDelete(true); }}
										style={{ color: "#e74c3c" }}
									>
										Delete
									</button>
				...
				>
					...
					<div className={styles.formGroup}>
						...
						<select
							...
							onChange={(e) =>
								setSelectedProjectScope(e.target.value as ProjectScope)
							}
						>
					...
					<div className={styles.formGroup}>
						...
						<div className={styles.customTagInput}>
							...
							/>
							<button type="button" onClick={handleAddCustomGenre}>
								Add
							</button>
						</div>
					</div>
			...
			{showSeriesCreate && (
				<SubDialog open={showSeriesCreate} onClose={() => setShowSeriesCreate(false)} title="Create Series">
					<div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
						<div>
							<label>Series Name</label>
							<input type="text" value={seriesCreateName} onChange={(e) => setSeriesCreateName(e.target.value)}
								onKeyDown={(e) => e.key === "Enter" && handleSeriesCreate()} autoFocus style={{ width: "100%" }} />
						</div>
						<div>
							<label>Description</label>
							<textarea value={seriesCreateDesc} onChange={(e) => setSeriesCreateDesc(e.target.value)} rows={3} style={{ width: "100%" }} />
						</div>
						<div>
							<label>Architecture</label>
							<select value={seriesCreateArch} onChange={(e) => setSeriesCreateArch(e.target.value as SeriesArchitecture)} style={{ width: "100%" }}>
						...
						<div style={{ display: "flex", justifyContent: "flex-end", gap: "0.5rem", marginTop: "0.5rem" }}>
							<button onClick={() => setShowSeriesCreate(false)}>Cancel</button>
							<button onClick={handleSeriesCreate} disabled={!seriesCreateName.trim()}>Create</button>
						</div>
					</div>
				</SubDialog>
			)}
			...
			{showSeriesEdit && seriesEditId && (
				<SubDialog open={showSeriesEdit} onClose={() => setShowSeriesEdit(false)} title="Edit Series">
					<div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
						<div>
							<label>Series Name</label>
							<input type="text" value={seriesEditName} onChange={(e) => setSeriesEditName(e.target.value)} style={{ width: "100%" }} />
						</div>
						<div>
							<label>Description</label>
							<textarea value={seriesEditDesc} onChange={(e) => setSeriesEditDesc(e.target.value)} rows={3} style={{ width: "100%" }} />
						</div>
						<div>
							<label>Architecture</label>
							<select value={seriesEditArch} onChange={(e) => setSeriesEditArch(e.target.value as SeriesArchitecture)} style={{ width: "100%" }}>
								{seriesArchOptions.map((o) => (
									<option key={o.value} value={o.value}>{o.label}</option>
								))}
							</select>
						...
						<div style={{ display: "flex", justifyContent: "flex-end", gap: "0.5rem", marginTop: "0.5rem" }}>
							<button onClick={() => setShowSeriesEdit(false)}>Cancel</button>
							<button onClick={handleSeriesEditSave} disabled={!seriesEditName.trim()}>Save</button>
						</div>
					</div>
				</SubDialog>
			)}
			...
			{showSeriesDelete && seriesDeleteId && (
				<SubDialog open={showSeriesDelete} onClose={() => setShowSeriesDelete(false)} title="Delete Series">
					<p>Are you sure you want to delete this series?</p>
					<div style={{ display: "flex", justifyContent: "flex-end", gap: "0.5rem", marginTop: "0.5rem" }}>
						<button onClick={() => setShowSeriesDelete(false)}>Cancel</button>
						<button onClick={handleSeriesDeleteConfirm} style={{ color: "#e74c3c" }}>Delete</button>
					</div>
				</SubDialog>
			)}

			{showSeriesProjects && (
				<SubDialog open={showSeriesProjects} onClose={() => setShowSeriesProjects(false)} title={`Projects in "${seriesProjectsName}"`}>
