...
function findBestMatch(aiFieldName: string, templateFields: FieldDefinition[]): FieldDefinition | null {
	...
	for (const tf of templateFields) {
		const tfNorm = normalizeName(tf.name);
		if (tfNorm.includes(normalized) || normalized.includes(tfNorm)) return tf;
	}
	return null;
}
...
export default function TemplateFieldMappingDialog({
	...
	const [mappings, setMappings] = useState<FieldMapping[]>(() =>
		aiFields.map((af) => {
			...
			return {
				aiField: af.name,
				action: match ? "map" as MappingAction : "add" as MappingAction,
				targetField: match ? match.name : null,
				newFieldDef: match ? null : {
					name: af.name,
					label: af.name.replace(/[_-]/g, " ").replace(/\b\w/g, (c) => c.toUpperCase()),
					type: "text" as const,
					required: false,
				},
			};
	...
	return (
		<Dialog open={open} onClose={() => onClose(null)} title={`Map Fields for ${entryName}`} large>
			<div style={{ display: "flex", flexDirection: "column", gap: "12px", flex: 1, overflow: "hidden" }}>
				<p style={{ color: "#aaa", fontSize: "13px", margin: 0 }}>
					The AI generated fields for this {categoryLabels[category].toLowerCase()} entry.
					Map them to template fields, add new persistent fields, or skip them.
				</p>
				...
				<div style={{ flex: 1, overflow: "auto", display: "flex", flexDirection: "column", gap: "6px" }}>
					{mappings.map((m) => {
						const aiField = aiFields.find((af) => af.name === m.aiField)!;
						const valueStr = typeof aiField.value === "string"
							? aiField.value.length > 80 ? aiField.value.slice(0, 80) + "..." : aiField.value
							: JSON.stringify(aiField.value);

						return (
							<div key={m.aiField} style={{
								display: "grid",
								gridTemplateColumns: "1fr 1.5fr 2.5fr",
								gap: "8px",
								alignItems: "start",
								...
								<code style={{ color: "#4A9EFF", fontSize: "13px", wordBreak: "break-all" }}>
									{m.aiField}
								</code>
								<span style={{ color: "#ccc", fontSize: "13px", wordBreak: "break-word", lineHeight: 1.4 }}>
									{valueStr}
								</span>
								<div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
									<select
										...
										onChange={(e) => {
											...
											if (action === "map" && !m.targetField && templateFieldNames.length > 0) {
												setMapping(m.aiField, { targetField: templateFieldNames[0] });
											}
											if (action === "add" && !m.newFieldDef) {
												setMapping(m.aiField, {
													newFieldDef: {
														name: m.aiField,
														label: m.aiField.replace(/[_-]/g, " ").replace(/\b\w/g, (c) => c.toUpperCase()),
														type: "text",
														required: false,
													},
												});
									...
									>
										<option value="map">Map to template field</option>
										<option value="add">Add as new field</option>
									...
									{m.action === "map" && (
										<select
											...
											value={m.targetField || ""}
											onChange={(e) => setMapping(m.aiField, { targetField: e.target.value || null })}
										>
									...
									{m.action === "add" && m.newFieldDef && (
										<div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
											<input
												...
												onChange={(e) => setMapping(m.aiField, {
													newFieldDef: { ...m.newFieldDef!, name: e.target.value },
												})}
											/>
											<div style={{ display: "flex", gap: "4px" }}>
												<input
													...
													onChange={(e) => setMapping(m.aiField, {
														newFieldDef: { ...m.newFieldDef!, label: e.target.value },
													})}
												/>
												<select
													style={{
													...
													onChange={(e) => setMapping(m.aiField, {
														newFieldDef: { ...m.newFieldDef!, type: e.target.value as FieldDefinition["type"] },
													})}
												>
													{fieldTypes.map((ft) => (
