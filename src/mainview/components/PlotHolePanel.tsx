...
import type { AnalysisIssue, AnalysisResult, AnalysisContext } from "../services/analysis/plotHoles";
...
import { IconAlertTriangle, IconAlertCircle, IconInfoCircle, IconRefresh, IconChevronDown, IconChevronRight } from "@tabler/icons-react";
...
export default function PlotHolePanel({ context, onNavigateChapter }: Props) {
	...
	const analyze = useCallback(async () => {
		...
		try {
			...
			setResult(r);
			const p = analyzePacing(context.chapters, context.acts, context.storyBeats);
			setInsights(p);
		} finally {
			setLoading(false);
		}
	...
	return (
		<div className="plot-hole-panel">
			<div className="php-header">
				...
				<button className="icon-btn" onClick={analyze} disabled={loading}>
			...
			{result && (
				<div className="php-summary">
					<div className={`summary-badge summary-error ${errorCount === 0 ? "ok" : ""}`}>
						{errorCount} errors
					</div>
					<div className={`summary-badge summary-warning ${warningCount === 0 ? "ok" : ""}`}>
			...
			<div className="php-issues">
				...
				{!loading && filteredIssues.length === 0 && (
					<div className="php-empty">
						{allIssues.length === 0 ? "No issues found. Your story structure looks solid!" : "No matching issues."}
					</div>
				)}
				{filteredIssues.map(issue => {
					...
					return (
						<div key={issue.id} className={`php-issue php-${issue.severity}`}>
							<div className="php-issue-header" onClick={() => toggleIssue(issue.id)}>
								<Icon size={16} color={SEVERITY_COLORS[issue.severity]} />
								<span className="php-issue-title">{issue.title}</span>
								<span className="php-issue-category">{issue.category}</span>
											...
											{issue.locations.map((loc, i) => (
												loc.chapterId && (
													<button
														...
														onClick={() => onNavigateChapter(loc.chapterId!)}
													>
														Go to chapter
													</button>
												)
