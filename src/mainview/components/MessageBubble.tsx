...
const MessageBubble = forwardRef<HTMLDivElement, MessageBubbleProps>(
	(
		...
		if (viewMode === "accordion" && isOld && !isExpanded) {
			...
			return (
				...
				>
					...
					<span className="accordion-preview-role">{message.role}</span>
				</div>
			);
		}
			...
			return (
				<div ref={ref} className="message-wrapper assistant" data-msg-id={message.id}>
					...
					<div className="chat-message assistant">
						...
						{isTruncated && (
							<button className="show-more-btn" onClick={() => onExpand(message.id)}>
								Show More
							</button>
						)}
					</div>
					<div className="chat-message-footer">
						<span className="message-meta">{modelDisplayText || message.model}</span>
						{totalVariants > 1 && (
							<div className="message-pagination">
								<button
									className="message-pagination-btn"
									onClick={() => onVariantChange(message.id, message.currentVariantIndex - 1)}
									disabled={message.currentVariantIndex === 0}
								>
									<IconChevronLeft size={12} />
								</button>
								<span className="message-pagination-label">
									{message.currentVariantIndex + 1} / {totalVariants}
								</span>
								<button
									className="message-pagination-btn"
									onClick={() => onVariantChange(message.id, message.currentVariantIndex + 1)}
									disabled={message.currentVariantIndex === totalVariants - 1}
								>
		...
		return (
			<div ref={ref} className="message-wrapper user" data-msg-id={message.id}>
