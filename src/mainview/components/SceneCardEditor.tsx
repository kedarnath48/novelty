...
export default function SceneCardEditor({ scene, index, characters, collapsed, onToggleCollapsed, onChange, onRemove, dragHandle, extraActions }: Props) {
	...
	function removeKeyEvent(idx: number) {
		onChange(scene.id, "keyEvents", JSON.stringify(keyEvents.filter((_, i) => i !== idx)));
	}
	...
	return (
		<div className={`scene-card${collapsed ? " collapsed" : ""}`}>
			<div className="scene-header">
				{dragHandle}
				<button className="collapse-btn" onClick={onToggleCollapsed} title={collapsed ? "Expand scene" : "Collapse scene"}>
					{collapsed ? <IconChevronRight size={12} /> : <IconChevronDown size={12} />}
				</button>
				<span className="scene-number">Scene {index + 1}</span>
				{extraActions}
				<button className="icon-btn icon-btn-sm" onClick={() => onRemove(scene.id)}><IconTrash size={12} /></button>
			</div>
			<input
			...
			<div className="scene-body">
				<textarea
					...
					value={scene.summary || ""}
					onChange={e => onChange(scene.id, "summary", e.target.value)}
					rows={2}
				/>
				<div className="scene-details">
					<input
						...
						value={scene.setting || ""}
						onChange={e => onChange(scene.id, "setting", e.target.value)}
					/>
					<input
						placeholder="Duration (e.g. 10 minutes)"
						value={scene.duration || ""}
						onChange={e => onChange(scene.id, "duration", e.target.value)}
					/>
					<input
						placeholder="Core conflict..."
						value={scene.conflict || ""}
						onChange={e => onChange(scene.id, "conflict", e.target.value)}
					/>
				</div>
				{characters.length > 0 && (
				...
				{keyEvents.length > 0 && (
					<div className="scene-key-events">
						...
						<button className="icon-btn" onClick={addKeyEvent}><IconPlus size={12} /> Add</button>
					</div>
				)}
			</div>
		</div>
