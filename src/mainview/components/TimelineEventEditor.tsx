...
export default function TimelineEventEditor({
	...
	return (
		<SubDialog open={open} onClose={onClose} title={event ? "Edit Event" : "New Event"}>
			<div style={{ display: "flex", flexDirection: "column", gap: "12px", minWidth: "400px" }}>
				<div>
					<label style={{ display: "block", marginBottom: "4px", fontSize: "12px", color: "#888" }}>Title</label>
					<input
						type="text"
						value={title}
						onChange={(e) => setTitle(e.target.value)}
						placeholder="Event title"
						style={{ width: "100%", padding: "6px 8px", background: "#1a1a1a", border: "1px solid #333", color: "#fff", borderRadius: "4px" }}
					/>
				</div>
				<div>
					<label style={{ display: "block", marginBottom: "4px", fontSize: "12px", color: "#888" }}>Description</label>
					<textarea
						value={description}
						onChange={(e) => setDescription(e.target.value)}
						placeholder="Event details..."
						rows={3}
						style={{ width: "100%", padding: "6px 8px", background: "#1a1a1a", border: "1px solid #333", color: "#fff", borderRadius: "4px", resize: "vertical" }}
					/>
				</div>
				<div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
					<div>
						<label style={{ display: "block", marginBottom: "4px", fontSize: "12px", color: "#888" }}>In-Story Date</label>
						<input
							type="text"
							value={inStoryDate}
					...
					<div>
						<label style={{ display: "block", marginBottom: "4px", fontSize: "12px", color: "#888" }}>Sort Order</label>
						<input
							type="number"
							value={dateOrder}
							onChange={(e) => setDateOrder(Number(e.target.value))}
							style={{ width: "100%", padding: "6px 8px", background: "#1a1a1a", border: "1px solid #333", color: "#fff", borderRadius: "4px" }}
						/>
					</div>
				</div>
				<div>
					<label style={{ display: "block", marginBottom: "4px", fontSize: "12px", color: "#888" }}>Event Type</label>
					<select
						value={eventType}
				...
				<div>
					<label style={{ display: "block", marginBottom: "4px", fontSize: "12px", color: "#888" }}>Linked Entity (optional)</label>
					<select
						value={entityId}
				...
				<div>
					<label style={{ display: "block", marginBottom: "4px", fontSize: "12px", color: "#888" }}>Linked Chapter (optional)</label>
					<select
						value={chapterId}
				...
				<div style={{ display: "flex", justifyContent: "flex-end", gap: "8px", marginTop: "8px" }}>
					<button className="btn" onClick={onClose} style={{ background: "#333" }}>Cancel</button>
