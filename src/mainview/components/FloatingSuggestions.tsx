...
export default function FloatingSuggestions({
	...
	const handlerRef = useRef<{ isOpen: boolean; groups: SuggestionGroup[]; onSelect: (id: string) => void; onClose: () => void }>(
	...
	useEffect(() => {
		const onKeyDown = (e: KeyboardEvent) => {
			...
			if (e.key === "ArrowDown") {
				e.preventDefault();
				setSelectedIndex((i) => Math.min(i + 1, h.groups.flatMap((g) => g.items).length - 1));
	...
	return (
		<div className="floating-suggestions" ref={listRef}>
			{groups.map((group, gi) => (
				<div key={group.label ?? gi} className="floating-suggestions-group">
					{group.label && (
					...
					{group.items.map((item) => {
						...
						return (
							...
							>
								<span className="floating-suggestions-item-primary">{item.primary}</span>
								{item.secondary && (
									<span className="floating-suggestions-item-secondary">{item.secondary}</span>
								)}
								{item.metadata && (
									<span className="floating-suggestions-item-metadata">{item.metadata}</span>
								)}
							</div>
						);
					})}
