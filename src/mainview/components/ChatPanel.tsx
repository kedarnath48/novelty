...
interface ChatPanelProps {
	...
	onCreateCompendiumEntry?: (category: CompendiumCategory, name: string, templateData: Record<string, unknown>) => Promise<string | null>;
	onExtractEntities?: (entries: ParsedEntry[], source: ExtractionSource) => void;
...
export default function ChatPanel({
	...
	const [activeView, setActiveView] = useState<"chat" | "history" | "archive">("chat");
	const [showSettings, setShowSettings] = useState(false);
	const [settingsRoute, setSettingsRoute] = useState<{ tab: string; section?: string; focus?: string } | undefined>(undefined);
	...
	const [fileAttachments, setFileAttachments] = useState<{ path: string; label: string }[]>([]);
	const [customSystemPrompt, setCustomSystemPrompt] = useState(project?.systemPrompt || "");
	...
	const EXTRACT_COMMANDS = new Set([
		"extractcharacters", "extractlocations", "extractorganizations",
		"extractitems", "extractlore", "extractall", "updatecompendium",
	]);
	...
	const SLASH_COMMANDS = [
		{ command: "/continue", description: "Continue writing from cursor", type: "instant" as const },
		{ command: "/rewrite", description: "Rewrite selected text", type: "instant" as const },
		{ command: "/expand", description: "Expand on selected text", type: "instant" as const },
		{ command: "/summarize", description: "Summarize selected text", type: "instant" as const },
		{ command: "/tone:", description: "Change tone (formal, casual, poetic...)", type: "context" as const },
		{ command: "/createcharacter", description: "Create a new character entry", type: "context" as const },
		{ command: "/createlocation", description: "Create a new location entry", type: "context" as const },
		{ command: "/createorganization", description: "Create a new organization entry", type: "context" as const },
		{ command: "/createitem", description: "Create a new item entry", type: "context" as const },
		{ command: "/createlore", description: "Create a new lore entry", type: "context" as const },
		{ command: "/extractcharacters", description: "Extract character entries from text", type: "context" as const },
		{ command: "/extractlocations", description: "Extract location entries from text", type: "context" as const },
		{ command: "/extractorganizations", description: "Extract organization entries from text", type: "context" as const },
		{ command: "/extractitems", description: "Extract item entries from text", type: "context" as const },
		{ command: "/extractlore", description: "Extract lore entries from text", type: "context" as const },
		{ command: "/extractall", description: "Extract all entity types from text", type: "context" as const },
		{ command: "/updatecompendium", description: "Update existing entries from text", type: "context" as const },
	];
	...
	const getEnabledModels = useCallback(() => {
		const provider = settings?.providers.defaultProvider
			...
			: null;
		if (provider?.models) {
			const displayMode = provider.modelDisplayMode ?? settings?.providers.modelDisplayMode ?? "alias";
			return Object.entries(provider.models)
				.filter(([, entry]) => typeof entry === "boolean" ? entry : entry.enabled)
				.map(([name, entry]) => {
					const alias = typeof entry === "object" ? entry.alias : undefined;
					const displayText = alias
						? displayMode === "both" ? `${alias} (${name})` : alias
						: name;
	...
	const getModelDisplayText = useCallback((modelName: string): string => {
		const provider = settings?.providers.defaultProvider
			? settings.providers.configs[settings.providers.defaultProvider]
			: null;
		if (provider?.models) {
			const entry = provider.models[modelName];
			const alias = entry && typeof entry === "object" ? entry.alias : undefined;
			if (alias) {
				const displayMode = provider.modelDisplayMode ?? settings?.providers.modelDisplayMode ?? "alias";
	...
	const streamingUpdate = useCallback((assistantId: string, content: string) => {
		setMessages((prev) => {
			const updated = [...prev];
	...
	const handleSubmit = async (prompt?: string) => {
		...
		let systemPromptMessage: { role: "system"; content: string } | null = null;
		if (project) {
			...
			while ((match = mentionPattern.exec(text)) !== null) {
				...
				const id = match[2];
				if (["chapter", "character", "location", "organization", "item", "lore"].includes(type)) {
					const existing = mentions.find((m) => m.id === id && m.type === type);
					parsedMentions.push(existing || { type, id, label: "", mode: "brief" });
				}
			}
			...
			for (const f of fileAttachments) {
				try {
					const content = await rpc.request["file:read-content"](f.path);
					if (content != null) fileContents.push(`\n[Attached file: "${f.label}"]\n${content}`);
				} catch { /* skip */ }
			}
		...
		let slashTarget: { category: CompendiumCategory; name: string } | null = null;
		...
		if (isCreateCommand || EXTRACT_COMMANDS.has(command)) {
			...
			if (EXTRACT_COMMANDS.has(command)) {
				...
				if (systemPromptMessage) {
					...
					const idField = isUpdateCommand ? ', "id": "existing-entry-id"' : "";
					systemPromptMessage.content =
						"You are analyzing text from the user's novel. " + categoryFilter + " " + actionInstruction + "\n\n" +
						"Read the text below carefully and output " + (isUpdateCommand ? "updates for matching entries" : "all entities you can identify") + ".\n\n" +
						existingContext + "\n\n" +
						"Each entry-data block must follow this format:\n" +
						bt + "entry-data\n" +
						'{"category": "character|location|organization|item|lore", "name": "Entity Name"' + idField + ', "fields": {"field1": "value1", ...}}\n' +
						bt + "\n\n" +
				...
				if (extractionSource?.text) {
					...
					displayText = "No text available for extraction. Open a chapter or select text first.";
					setIsLoading(false);
				...
				const cmdName = commaIdx > 0 ? cmdArgs.substring(0, commaIdx).trim() : cmdArgs.trim();
				const description = commaIdx > 0 ? cmdArgs.substring(commaIdx + 1).trim() : "";
		...
		try {
				...
				onChunk: (chunk) => {
					...
					if (rafIdRef.current === null) {
						rafIdRef.current = requestAnimationFrame(() => {
							...
							streamingUpdate(assistantId, streamedContentRef.current);
						});
					}
				},
			...
			if (!sessionIsManuallyNamed) {
				let newTitle: string;
				const autoMethod = settings?.general.autoNamingMethod ?? "smart-truncation";
				...
				if (autoMethod === "ai-summarizer") {
					try {
						...
						const recentApiMessages = toApiMessages(
							[...messages, userMessage, completedAssistant].slice(-4),
						);
						if (systemPromptMessage) {
						...
						newTitle = titleResult.content.trim().replace(/^["']|["']$/g, "").substring(0, 60);
			...
			if (isExtractCommand || isUpdateCommand) {
				...
				if (parsed) {
					setPendingEntryData((prev) => ({ ...prev, [assistantId]: parsed }));
					if (slashTarget) {
						setSlashCommandTargets((prev) => ({ ...prev, [assistantId]: slashTarget }));
					}
				}
			}
		} catch (error) {
			console.error("Chat error:", error);
			streamingUpdate(assistantId, "Error: Failed to get response from AI");
		} finally {
			isStreamingRef.current = false;
			setIsLoading(false);
		}
			...
			const mapped: ChatMessage[] = (msgs || []).map((m) => {
				if (m.role === "assistant") {
					return {
						...
						timestamp: typeof m.timestamp === "string" ? m.timestamp : m.timestamp?.toISOString?.() || new Date(m.timestamp).toISOString(),
					} as AssistantChatMessage;
				}
				return {
					id: m.id,
	...
	const handleOpenSettingsProviders = () => {
		setSettingsRoute({ tab: "providers", section: "add-provider", focus: "provider-id" });
	...
	const handleDeleteSession = useCallback(
		async (id: string) => {
			...
			if (session.isArchived) {
				alert("This session is vaulted. You must unvault it before it can be deleted.");
				return;
			}
	...
	const handleRetry = async (messageId: string) => {
		...
		setMessages((prev) =>
			prev.map((m) =>
				m.id === messageId && m.role === "assistant"
					? { ...m, content: [...m.content, ""], currentVariantIndex: newVariantIndex }
					: m,
			),
		);
		...
		try {
					...
					if (rafIdRef.current === null) {
						rafIdRef.current = requestAnimationFrame(() => {
							...
							setMessages((prev) =>
								prev.map((m) =>
									m.id === messageId && m.role === "assistant"
										? { ...m, content: [...m.content.slice(0, newVariantIndex), content] }
										: m,
								),
							);
						});
			...
			const parsed = parseEntryData(result.content);
			if (parsed) {
				setPendingEntryData((prev) => ({ ...prev, [messageId]: parsed }));
			}
		} catch (error) {
			...
			setMessages((prev) =>
				prev.map((m) =>
					m.id === messageId && m.role === "assistant"
						? { ...m, content: [...m.content.slice(0, newVariantIndex), "Error: Request failed"] }
						: m,
				),
			);
		} finally {
	...
	const handleAtSelect = useCallback((target: MentionTarget) => {
		...
		setMentions((prev) => {
			if (prev.some((m) => m.id === target.id && m.type === target.type)) return prev;
	...
	const handleSuggestionsSelect = useCallback((id: string) => {
		if (showSlashPopup) {
	...
	const suggestionGroups = useMemo((): SuggestionGroup[] => {
		if (showSlashPopup) {
			const filtered = SLASH_COMMANDS.filter(
				(cmd) => !slashSearchTerm || cmd.command.toLowerCase().startsWith(`/${slashSearchTerm.toLowerCase()}`)
			);
			return [{
				items: filtered.map((cmd) => ({
					id: cmd.command,
					primary: <span className={`fs-type-${cmd.type}`}>{cmd.command}</span>,
		...
		if (showAtPopup) {
			const each = <T extends { id: string; name: string }>(
				...
				const items = list
					.filter((i) => !atSearchTerm || i.name.toLowerCase().includes(atSearchTerm.toLowerCase()))
					.map((i) => ({
						id: JSON.stringify({ type, id: i.id, label: i.name }),
						primary: i.name,
						...(opts?.metadata ? { metadata: opts.metadata } : {}),
					}));
				return items.length > 0 ? { label: typeLabels[type], icon: typeIcons[type], items } : null;
			};

			return [
				each(chapters.map(c => ({ id: c.id, name: c.title })), "chapter" as const, { metadata: "Brief" }),
		...
		return [];
	}, [showSlashPopup, showAtPopup, slashSearchTerm, atSearchTerm, chapters, characters, locations, organizations, items, loreEntries]);
	...
	const handleToggleMentionMode = (id: string) => {
		setMentions((prev) =>
			prev.map((m) =>
				m.id === id ? { ...m, mode: m.mode === "full" ? "brief" as const : "full" as const } : m,
			),
		);
	};
	...
	const handleApply = (content: string) => {
		...
		if (trimmed.startsWith("/rewrite") || trimmed.startsWith("/tone:")) {
			editorRef.current.replaceSelection(trimmed.replace(/^\/\w+\:?\S*\s*/, ""));
		} else {
	...
	const handleInputChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
		...
		if (showSlashPopup && slashTriggerPos.current !== null) {
			...
			const term = spaceIdx >= 0 ? slashText.slice(0, spaceIdx) : slashText;
		...
		if (showAtPopup && atTriggerPos.current) {
			...
			// Close if no @ context remains
			if (!value.includes("@") || (atTriggerPos.current.start > value.length)) {
				setShowAtPopup(false);
				atTriggerPos.current = null;
			}
		}
		...
		<div className="chat-panel" style={style}>
			<div className="chat-panel-header">
				...
				<div className="chat-panel-header-actions">
					...
					<button onClick={handleOpenSettingsProviders} title="Settings">
				...
				{activeView === "chat" && (
					<div className="chat-panel-messages" ref={messagesRef}>
						{messages.length === 0 && (
							<div className="chat-welcome">
								...
								<p className="chat-hint">
									Try: "Describe the main character's appearance" or
						...
						{messages.map((msg, i) => {
							...
							const effectiveCategory = slashOverride?.category || entryData?.category;
							const effectiveName = slashOverride?.name || entryData?.name;
							const createEntryBtn = entryData && effectiveCategory && effectiveName && onCreateCompendiumEntry ? (
								<button
									className="message-action-btn create-entry-btn"
									onClick={async () => {
										setCreatingEntryId(msg.id);
										try {
											await onCreateCompendiumEntry(effectiveCategory!, effectiveName!, entryData.templateData);
								...
								>
									{creatingEntryId === msg.id ? <IconLoader2 size={14} className="spin" /> : <IconBook size={14} />}
									<span style={{ fontSize: "11px", marginLeft: "2px" }}>{effectiveCategory}</span>
								</button>
							) : undefined;
							return (
								<MessageBubble
									...
									message={msg}
									modelDisplayText={msg.role === "assistant" ? getModelDisplayText((msg as AssistantChatMessage).model) : undefined}
							...
							return (
								<div className="editor-apply-bar">
									<button
										...
										onClick={() => handleApply(last.content[last.currentVariantIndex])}
									>
										Apply to editor
									</button>
									<button
						...
						{isLoading && messages.length > 0 && (() => {
							const last = messages[messages.length - 1];
							if (last.role === "assistant" && last.content[0].length === 0) {
								return (
						...
						<div className="scroll-nav-buttons">
							{showScrollTop && (
								<button className="scroll-nav-btn" onClick={scrollToTop} title="Scroll to top">
									<IconArrowUp size={16} />
								</button>
							)}
							{showJumpToPrompt && (
								<button className="scroll-nav-btn" onClick={jumpToPrompt} title="Jump to prompt">
									<IconArrowBackUp size={16} />
								</button>
							)}
							{showScrollBottom && (
								<button className="scroll-nav-btn" onClick={scrollToBottom} title="Scroll to bottom">
				...
				<div className="chat-panel-input">
					...
					{mode === "editor" && (
						<div className="editor-actions">
							<button className="editor-action-btn" onClick={() => setInput("/continue ")} title="Continue writing from cursor">
								Continue
							</button>
							<button className="editor-action-btn" onClick={() => setInput("/rewrite ")} title="Rewrite selected text">
								Rewrite
							</button>
							<button className="editor-action-btn" onClick={() => setInput("/expand ")} title="Expand on selected text">
								Expand
							</button>
							<button className="editor-action-btn" onClick={() => setInput("/summarize ")} title="Summarize">
								Summarize
							</button>
							<select
					...
					{voice.isListening && (
						<div className="voice-indicator">
							...
							<span className="voice-label">
								{voiceInterim ? `"${voiceInterim}"` : "Listening..."}
							</span>
						</div>
					)}
					<div className="chat-input-wrapper">
						...
						<textarea
							...
							placeholder={mode === "editor" ? "Describe what to write, or use /continue, /rewrite..." : "Ask anything... @ to mention"}
						/>
					</div>
					<div className="chat-panel-footer">
						<div className="chat-panel-footer-left">
							<button title="Attach file" onClick={handleAttachFile}>
						...
						<div className="chat-panel-footer-right">
							<button
								...
								className={voice.isListening ? "voice-active" : ""}
