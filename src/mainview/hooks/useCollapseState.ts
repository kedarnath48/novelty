...
const EMPTY_STATE: CollapseState = { acts: [], chapters: [], sequences: [], scenes: [] };
...
function loadState(projectId: string): CollapseState {
	try {
		...
		return {
			...
			chapters: Array.isArray(parsed.chapters) ? parsed.chapters.map(String) : [],
			sequences: Array.isArray(parsed.sequences) ? parsed.sequences.map(String) : [],
...
export function useCollapseState(projectId: string) {
	const [collapsed, setCollapsed] = useState<CollapseState>(() => loadState(projectId));
	...
	const isCollapsed = (kind: CollapseKind, id: string) => collapsed[kind].includes(id);
