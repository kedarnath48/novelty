...
function App() {
  ...
  const [activeDialog, setActiveDialog] = useState<
    "projects" | "settings" | "ui" | "openFileDialog" | "projectManager" | null
  >(null);
  ...
  async function restoreTabContents(tabs: FileTab[]) {
    for (const tab of tabs) {
      if (tab.type === "chapter") {
        ...
        if (c?.content) {
          setTabContents((prev) => ({ ...prev, [tab.id]: c.content! }));
        }
        ...
        switch (tab.category) {
          ...
          case "location":
            ...
            break;
          case "organization":
            entry = await rpc.request["db:get-organization"](tab.id);
            break;
          case "item":
        ...
        if (entry) {
          setCompendiumEntries((prev) => ({ ...prev, [tab.id]: entry }));
        }
      } else if (tab.type === "file" && tab.filePath) {
        const content = await rpc.request["file:read-content"](tab.filePath);
  ...
  async function handleSelectProject(projectId: string) {
    if (scratchSaveTimeoutRef.current) clearTimeout(scratchSaveTimeoutRef.current);
  ...
  async function loadTemplateForCategory(
    ...
    const [template, resolved] = await Promise.all([
      ...
      rpc.request["db:get-resolved-template"]({ projectId, baseType: category }),
    ]);
    setTemplates((prev) => ({ ...prev, [category]: template || null }));
    setResolvedTemplateFields((prev) => ({ ...prev, [category]: resolved?.fields || [] }));
  }
  ...
  async function saveChapterContent(tabId: string, content: string) {
    ...
    justSavedTimerRef.current = setTimeout(() => {
      setSaveState((prev) => {
        ...
        return modifiedTabIdsRef.current.has(tabId) ? "idle-modified" : "idle-unmodified";
      });
    }, 2000);
  }

  const saveTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const scratchSaveTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  async function handleNewChapter() {
    if (!currentProject) return;
    const chapterList = await rpc.request["db:get-chapters"](currentProject.id);
    ...
    if (newEntry && (newEntry as { id: string }).id) {
      ...
      switch (category) {
        ...
        case "location":
          ...
          break;
        case "organization":
          setOrganizations((prev) => [...prev, newEntry as Organization]);
          break;
        case "item":
    ...
    try {
      ...
      if (entry && (entry as { id: string }).id) {
        ...
        if (!existingTab) {
          ...
          const newTab: FileTab = {
            id: entryId,
            name: (entry as { name: string }).name || `New ${label}`,
            type: category,
            filePath: "",
            isModified: false,
            category,
  ...
  async function handleUpdateCompendiumEntry(
    ...
    try {
      ...
      if (updated) {
        setCompendiumEntries((prev) => ({ ...prev, [entryId]: updated }));
        setOpenTabs((prev) =>
          prev.map((t) =>
  ...
  async function handleCreateCompendiumEntryFromAI(
    ...
    try {
      ...
      const resolved = await rpc.request["db:get-resolved-template"]({ projectId: currentProject.id, baseType: category });
      ...
      const unmatchedKeys = aiFieldKeys.filter((key) => !templateFieldNames.has(key));
      ...
      if (unmatchedKeys.length > 0 && currentProject) {
        ...
        const mappingResult = await new Promise<MappingResult | null>((resolve) => {
        ...
        if (templateUpdates.length > 0) {
          const existingTemplate = await rpc.request["db:get-template"]({ projectId: currentProject.id, baseType: category });
          const updatedFields = [...(existingTemplate?.customFields || [])];
          for (const newField of templateUpdates) {
            if (!updatedFields.find((f) => f.name === newField.name)) {
          ...
          await rpc.request["db:save-template"]({
            ...
            globalTemplateId: existingTemplate?.globalTemplateId ?? undefined,
            seriesTemplateId: existingTemplate?.seriesTemplateId ?? undefined,
          });
          // Refresh resolved fields
          const newResolved = await rpc.request["db:get-resolved-template"]({ projectId: currentProject.id, baseType: category });
          setResolvedTemplateFields((prev) => ({ ...prev, [category]: newResolved?.fields || [] }));
        }
      }
  ...
  async function createCompendiumEntry(
    ...
    try {
      ...
      switch (category) {
        case "character":
          newEntry = await rpc.request["db:create-character"]({
            id, projectId: currentProject.id, name, filePath: null, templateData,
          });
          break;
        case "location":
          newEntry = await rpc.request["db:create-location"]({
      ...
      switch (category) {
        case "character": setCharacters((prev) => [...prev, newEntry as Character]); break;
        case "location": setLocations((prev) => [...prev, newEntry as Location]); break;
        case "organization": setOrganizations((prev) => [...prev, newEntry as Organization]); break;
        case "item": setItems((prev) => [...prev, newEntry as Item]); break;
        case "lore": setLoreEntries((prev) => [...prev, newEntry as LoreEntry]); break;
      }

      const newTab: FileTab = { id, name, type: category, filePath: "", isModified: false, category };
      setOpenTabs((prev) => [...prev, newTab]);
      setActiveTabId(id);
      return id;
    } catch (e) {
  ...
  async function handleExtractEntities(entries: ParsedEntry[], _source: ExtractionSource) {
  ...
  async function handleBulkExtractResult(result: BulkExtractResult | null) {
    ...
    for (const entry of result.entries) {
      if (entry.existingId) {
        ...
        const data = { name: entry.name, templateData: entry.templateData } as any;
        switch (entry.category) {
          case "character": await rpc.request["db:update-character"]({ id: entry.existingId, data }); break;
          case "location": await rpc.request["db:update-location"]({ id: entry.existingId, data }); break;
          case "organization": await rpc.request["db:update-organization"]({ id: entry.existingId, data }); break;
          case "item": await rpc.request["db:update-item"]({ id: entry.existingId, data }); break;
          case "lore": await rpc.request["db:update-lore-entry"]({ id: entry.existingId, data }); break;
        }
      } else {
        await createCompendiumEntry(entry.category, entry.name, entry.templateData);
      }
    }
    ...
    for (const update of result.templateUpdates) {
      const existing = await rpc.request["db:get-template"]({ projectId: currentProject.id, baseType: update.baseType });
      ...
      const newResolved = await rpc.request["db:get-resolved-template"]({ projectId: currentProject.id, baseType: update.baseType });
      setResolvedTemplateFields((prev) => ({ ...prev, [update.baseType]: newResolved?.fields || [] }));
    }

    setExtractedEntries([]);
  }

  async function handleUpdateCompendium(_entries: ParsedEntry[], _source: ExtractionSource) {
    ...
    try {
      switch (category) {
        case "character":
          ...
          setCharacters((prev) => prev.filter((e) => e.id !== entryId));
          break;
        case "location":
          await rpc.request["db:delete-location"](entryId);
          setLocations((prev) => prev.filter((e) => e.id !== entryId));
          break;
        case "organization":
          await rpc.request["db:delete-organization"](entryId);
          setOrganizations((prev) => prev.filter((e) => e.id !== entryId));
          break;
        case "item":
  ...
  function renderEditor() {
          ...
          <ChapterOutlineEditor
            ...
            onUpdate={async (field, value) => {
              if (field === "plotThreads") {
                ...
                await rpc.request["db:set-chapter-plot-threads"]({ chapterId: chapter.id, threads });
                setChapterPlotThreads(threads.map(t => ({
              ...
              await rpc.request["db:update-chapter"]({ id: chapter.id, data: { [field]: value } as any });
              const updated = await rpc.request["db:get-chapter"](chapter.id);
              if (updated) {
            ...
            onMoveToDraft={async () => {
              await rpc.request["db:update-chapter"]({ id: chapter.id, data: { status: "draft" } as any });
              const updated = await rpc.request["db:get-chapter"](chapter.id);
              if (updated) {
                setChapters(prev => prev.map(c => c.id === chapter.id ? updated : c));
                setTabContents(prev => ({
    ...
    if (entry) {
      ...
      const effectiveTemplate = resolvedFields.length > 0
        ? { ...templates[category], customFields: resolvedFields } as EntityTemplate
        : templates[category];
      return (
        <CompendiumEntryEditor
          entry={entry as Parameters<typeof CompendiumEntryEditor>[0]["entry"]}
          category={category}
  ...
  return (
    <>
      ...
      {activeDialog === "settings" && (
        <SettingsDialog open={true} onClose={() => setActiveDialog(null)} />
      )}
      {activeDialog === "ui" && (
      ...
      {activeDialog === "openFileDialog" && (
        <OpenFileDialog
          ...
          onFileSelect={async (absolutePath, _relativePath, fileName) => {
            ...
            const content = await rpc.request["file:read-content"](absolutePath);
            if (content !== undefined && content !== null) {
        ...
        <div className="analysis-overlay">
          <div className="analysis-panel">
            <div className="analysis-panel-header">
              ...
              <button className="icon-btn" onClick={() => setShowAnalysis(false)}>Close</button>
            </div>
            <PlotHolePanel
              context={{
                chapters,
      ...
      {mappingDialogOpen && pendingCreateEntry && (
        <TemplateFieldMappingDialog
          ...
          aiFields={Object.entries(pendingCreateEntry.templateData).map(([name, value]) => ({ name, value }))}
