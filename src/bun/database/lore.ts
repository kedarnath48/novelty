...
export async function getLoreEntriesByProject(projectId: string): Promise<LoreEntry[]> {
	const rows = await db
		.select()
		.from(loreEntries)
...
export async function createLoreEntry(entry: NewLoreEntry): Promise<LoreEntry> {
	...
	const newEntry = {
		...entry,
		templateData: entry.templateData ? JSON.stringify(entry.templateData) : null,
		createdAt: now,
		updatedAt: now,
	};
