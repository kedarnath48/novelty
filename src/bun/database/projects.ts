...
import { projects, projectGenres, projectTags, projectThemes, genres, tags, themes } from "../schema";
import { eq, asc, desc, or } from "drizzle-orm";
import { getAllSettings, setSetting } from "../settings";
import { join } from "path";
import { mkdirSync, existsSync } from "fs";
import type { ProjectScope, SeriesArchitecture, PointOfView, PacingType, WorkType, ProjectStructure, TargetAgeGroup, ProjectStatus } from "../../mainview/types";
...
function parseProject(row: typeof projects.$inferSelect): Project {
	return {
		...
		projectStructure: (row.projectStructure || null) as ProjectStructure | null,
...
async function resolveProjectJunctions(row: typeof projects.$inferSelect, parsed: Project) {
	const genreRows = await db.select({ name: genres.name, isPrimary: projectGenres.isPrimary })
		.from(projectGenres)
	...
	const themeRows = await db.select({ name: themes.name, isPrimary: projectThemes.isPrimary })
		.from(projectThemes)
...
export async function getAllProjects(): Promise<Project[]> {
	...
	const projectsWithMeta = await Promise.all(rows.map(async (row) => {
...
export async function createProject(project: NewProject): Promise<Project> {
	...
	if (projectsDir && project.id) {
		...
		if (!existsSync(projectPath)) {
			...
			mkdirSync(join(projectPath, "Compendium", "characters"), { recursive: true });
			mkdirSync(join(projectPath, "Compendium", "locations"), { recursive: true });
			mkdirSync(join(projectPath, "Compendium", "organizations"), { recursive: true });
			mkdirSync(join(projectPath, "Compendium", "items"), { recursive: true });
			mkdirSync(join(projectPath, "References"), { recursive: true });
		}
		const updatedProjectDirs = [...settings.projects.projectDirs, projectPath];
		setSetting("projects", { ...settings.projects, projectDirs: updatedProjectDirs });
	}

	const { genres: _pg, tags: _pt, themes: _pth, primaryGenre: _pG, primaryTheme: _pT, ...projectRest } = project;
...
export async function updateProject(
	id: string,
	data: Partial<NewProject> & { primaryGenre?: string | null; primaryTheme?: string | null; }
): Promise<Project | undefined> {
	const { genres: newGenres, tags: newTags, themes: newThemes, primaryGenre, primaryTheme, ...rest } = data;
	...
	if (newGenres) {
		const genreRows = await db.select()
			.from(genres)
			.where(or(...newGenres.map((name: string) => eq(genres.name, name))));
	...
	if (newTags) {
		...
		if (tagRows.length > 0) {
			await db.insert(projectTags).values(
				tagRows.map((t) => ({ projectId: id, tagId: t.id }))
			);
		}
	}
	...
	if (newThemes) {
		const themeRows = await db.select()
			.from(themes)
			.where(or(...newThemes.map((name: string) => eq(themes.name, name))));
