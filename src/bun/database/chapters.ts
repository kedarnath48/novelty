...
export async function createChapter(chapter: NewChapter): Promise<Chapter> {
	...
	return parseChapter({ ...chapter, createdAt: now, updatedAt: now, status: chapter.status || "outline" });
}
...
export async function updateChapter(
	...
	if (data.povCharacterId !== undefined) updateData.povCharacterId = data.povCharacterId;
	if (data.wordCountTarget !== undefined) updateData.wordCountTarget = data.wordCountTarget;
	if (data.actId !== undefined) updateData.actId = data.actId;
	if (data.sequenceId !== undefined) updateData.sequenceId = data.sequenceId;
	await db.update(chapters).set(updateData as any).where(eq(chapters.id, id));
