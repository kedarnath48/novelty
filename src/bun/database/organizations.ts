...
export async function createOrganization(organization: NewOrganization): Promise<Organization> {
	const now = new Date();
	const newOrg = {
		...organization,
		templateData: organization.templateData ? JSON.stringify(organization.templateData) : null,
		createdAt: now,
		updatedAt: now,
	};
