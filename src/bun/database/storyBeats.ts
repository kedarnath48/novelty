...
export async function getBeatsByProject(projectId: string): Promise<StoryBeat[]> {
	const rows = await db
		.select()
		.from(storyBeats)
...
export async function getBeatById(id: string): Promise<StoryBeat | undefined> {
	const result = await db.select().from(storyBeats).where(eq(storyBeats.id, id));
...
export async function createBeat(beat: NewStoryBeat): Promise<StoryBeat> {
	const now = new Date();
	await db.insert(storyBeats).values({ ...beat, createdAt: now, updatedAt: now });
	return { ...beat, createdAt: now, updatedAt: now };
}

export async function updateBeat(id: string, data: Partial<NewStoryBeat>): Promise<StoryBeat | undefined> {
