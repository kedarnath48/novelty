...
function parseInspiration(row: Record<string, unknown>): Inspiration {
	return {
		...row,
		inspiredAspects: row.inspiredAspects ? JSON.parse(row.inspiredAspects as string) : [],
		sourceYear: row.sourceYear ?? null,
	} as Inspiration;
}

export async function getInspirations(projectId: string): Promise<Inspiration[]> {
	const rows = await db
		.select()
		.from(inspirations)
...
export async function getInspirationById(id: string): Promise<Inspiration | undefined> {
	const result = await db.select().from(inspirations).where(eq(inspirations.id, id));
	if (!result[0]) return undefined;
	return parseInspiration(result[0]);
}

export async function createInspiration(data: NewInspiration): Promise<Inspiration> {
	const now = new Date();
	const row = {
		...data,
		inspiredAspects: data.inspiredAspects.length ? JSON.stringify(data.inspiredAspects) : null,
		createdAt: now,
		updatedAt: now,
	};
...
export async function updateInspiration(
	...
	if (data.inspiredAspects !== undefined)
		updateData.inspiredAspects = data.inspiredAspects.length ? JSON.stringify(data.inspiredAspects) : null;
	if (data.inspiredNotes !== undefined) updateData.inspiredNotes = data.inspiredNotes;
