...
export async function getAgentsByProject(projectId: string): Promise<Agent[]> {
	return db.select().from(agents).where(eq(agents.projectId, projectId)).orderBy(asc(agents.updatedAt));
}
...
export async function updateAgent(id: string, data: Partial<NewAgent>): Promise<Agent | undefined> {
...
export async function getRunsByAgent(agentId: string): Promise<AgentRun[]> {
	return db.select().from(agentRuns).where(eq(agentRuns.agentId, agentId)).orderBy(asc(agentRuns.startedAt));
}
...
export async function updateRun(id: string, data: Partial<NewAgentRun>): Promise<AgentRun | undefined> {
