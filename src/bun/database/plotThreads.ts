...
export async function getThreadsByProject(projectId: string): Promise<PlotThread[]> {
	const rows = await db
		.select()
		.from(plotThreads)
...
export async function getThreadById(id: string): Promise<PlotThread | undefined> {
	const result = await db.select().from(plotThreads).where(eq(plotThreads.id, id));
...
export async function createThread(thread: NewPlotThread): Promise<PlotThread> {
	const now = new Date();
	await db.insert(plotThreads).values({ ...thread, createdAt: now, updatedAt: now });
	return { ...thread, createdAt: now, updatedAt: now };
}

export async function updateThread(id: string, data: Partial<NewPlotThread>): Promise<PlotThread | undefined> {
...
export async function getChapterThreads(chapterId: string): Promise<ChapterPlotThread[]> {
	const rows = await db
		.select()
		.from(chapterPlotThreads)
		.where(eq(chapterPlotThreads.chapterId, chapterId));
	return rows.map(r => ({ chapterId: r.chapterId, plotThreadId: r.plotThreadId, intensity: r.intensity }));
}
...
export async function setChapterThreads(
	...
	await db.delete(chapterPlotThreads).where(eq(chapterPlotThreads.chapterId, chapterId));
	if (threads.length > 0) {
		await db.insert(chapterPlotThreads).values(
			threads.map(t => ({ chapterId, plotThreadId: t.plotThreadId, intensity: t.intensity })),
		);
	}
}
