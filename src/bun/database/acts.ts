...
export async function getActById(id: string): Promise<StoryAct | undefined> {
	const result = await db.select().from(storyActs).where(eq(storyActs.id, id));
...
export async function createAct(act: NewStoryAct): Promise<StoryAct> {
	const now = new Date();
	await db.insert(storyActs).values({ ...act, createdAt: now, updatedAt: now });
	return { ...act, createdAt: now, updatedAt: now };
}

export async function updateAct(id: string, data: Partial<NewStoryAct>): Promise<StoryAct | undefined> {
...
export async function getSequencesByProject(projectId: string): Promise<StorySequence[]> {
	const rows = await db
		.select()
		.from(storySequences)
...
export async function getSequencesByAct(actId: string): Promise<StorySequence[]> {
	const rows = await db
		.select()
		.from(storySequences)
...
export async function getSequenceById(id: string): Promise<StorySequence | undefined> {
	const result = await db.select().from(storySequences).where(eq(storySequences.id, id));
	if (!result[0]) return undefined;
	return parseSequence(result[0]);
}

export async function createSequence(seq: NewStorySequence): Promise<StorySequence> {
	const now = new Date();
	await db.insert(storySequences).values({ ...seq, createdAt: now, updatedAt: now });
	return { ...seq, createdAt: now, updatedAt: now };
}

export async function updateSequence(id: string, data: Partial<NewStorySequence>): Promise<StorySequence | undefined> {
	const updateData: Record<string, unknown> = { updatedAt: new Date() };
	if (data.title !== undefined) updateData.title = data.title;
	if (data.summary !== undefined) updateData.summary = data.summary;
	if (data.orderIndex !== undefined) updateData.orderIndex = data.orderIndex;
