...
export async function getScenesByProject(projectId: string): Promise<StoryScene[]> {
	const rows = await db
		.select()
		.from(storyScenes)
...
export async function getSceneById(id: string): Promise<StoryScene | undefined> {
	const result = await db.select().from(storyScenes).where(eq(storyScenes.id, id));
	if (!result[0]) return undefined;
	return parseScene(result[0]);
}

export async function getScenesBySequence(sequenceId: string): Promise<StoryScene[]> {
	const rows = await db
		.select()
		.from(storyScenes)
...
export async function getScenesByChapter(chapterId: string): Promise<StoryScene[]> {
	const rows = await db
		.select()
		.from(storyScenes)
...
export async function createScene(scene: NewStoryScene): Promise<StoryScene> {
	const now = new Date();
	await db.insert(storyScenes).values({ ...scene, createdAt: now, updatedAt: now });
	return { ...scene, createdAt: now, updatedAt: now };
}

export async function updateScene(id: string, data: Partial<NewStoryScene>): Promise<StoryScene | undefined> {
	const updateData: Record<string, unknown> = { updatedAt: new Date() };
	if (data.title !== undefined) updateData.title = data.title;
	if (data.summary !== undefined) updateData.summary = data.summary;
	if (data.setting !== undefined) updateData.setting = data.setting;
	if (data.charactersPresent !== undefined) updateData.charactersPresent = data.charactersPresent;
...
export async function reorderScenes(updates: { id: string; orderIndex: number }[]): Promise<void> {
...
export async function moveScene(id: string, data: { sequenceId?: string | null; chapterId?: string | null; actId?: string | null; orderIndex?: number }): Promise<StoryScene | undefined> {
	const updateData: Record<string, unknown> = { updatedAt: new Date() };
	if (data.sequenceId !== undefined) updateData.sequenceId = data.sequenceId;
	if (data.chapterId !== undefined) updateData.chapterId = data.chapterId;
	if (data.actId !== undefined) updateData.actId = data.actId;
