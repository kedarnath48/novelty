...
function mergeParagraphs(paragraphs: string[], maxTokens: number): TextChunk[] {
	...
	for (const para of paragraphs) {
		...
		if (countTokens(test) > maxTokens && current) {
			chunks.push({ text: current.trim(), tokenCount: countTokens(current.trim()) });
			current = para;
		} else {
			current = test;
		}
...
function chunkPlainText(text: string, maxTokens: number, overlapTokens: number): TextChunk[] {
...
export function chunkText(text: string, maxTokens: number, overlapTokens: number): TextChunk[] {
	...
	for (let i = 1; i < chunks.length; i++) {
		...
		overlapped.push({ text: combined.trim(), tokenCount: countTokens(combined.trim()) });
	}

	return overlapped;
}

export function chunkHtml(html: string, maxTokens: number, overlapTokens: number): TextChunk[] {
