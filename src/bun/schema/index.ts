...
export const chatSessions = sqliteTable("chat_sessions", {
	...
	isManuallyNamed: integer("is_manually_named", { mode: "boolean" }).notNull().default(false),
	createdAt: integer("created_at", { mode: "timestamp" }).notNull().$defaultFn(() => new Date()),
	updatedAt: integer("updated_at", { mode: "timestamp" }).notNull().$defaultFn(() => new Date()),
});
...
export const projectGenres = sqliteTable("project_genres", {
	...
	genreId: text("genre_id").references(() => genres.id, { onDelete: "cascade" }),
...
export const entityTemplates = sqliteTable("entity_templates", {
	...
	seriesTemplateId: text("series_template_id").references(() => seriesTemplates.id),
	customFields: text("custom_fields"),
	createdAt: integer("created_at", { mode: "timestamp" }).notNull().$defaultFn(() => new Date()),
	updatedAt: integer("updated_at", { mode: "timestamp" }).notNull().$defaultFn(() => new Date()),
});
...
export const chapterPlotThreads = sqliteTable("chapter_plot_threads", {
	...
	plotThreadId: text("plot_thread_id").notNull().references(() => plotThreads.id, { onDelete: "cascade" }),
